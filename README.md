...
- `apps/web/tests/config/responsiveManualMatrix.json` must exist (when `responsiveManualMatrixContract.test.ts` is active) and remain a strict manual QA matrix config (canonical top-level/user-route keys, canonical viewport/route lists, valid fixed-route regex patterns).
