    ...
    if ((existing.rowCount ?? 0) > 0) {
      ...
      if (publicationTypes && publicationTypes.length > 0) {
        const existingTypes: string[] =
          existing.rows[0].publication_types || [];
...
async function findOrCreateUniversalArticle(
...
async function createArticleFromCrossref(
  doi: string,
): Promise<{ articleId: string; created: boolean } | null> {
  // Нормализуем DOI
  const normalizedDoi = doi.toLowerCase().trim();

  // Проверяем, есть ли уже статья с таким DOI
  const existing = await pool.query(`SELECT id FROM articles WHERE doi = $1`, [
    normalizedDoi,
  ]);
...
const plugin: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      if (!bodyP.success) {
        return reply
          .code(400)
          .send({ error: "Invalid body", details: bodyP.error.message });
      }
    ...
    async (request, reply) => {
      ...
      const existingPmidsInProject = new Set<string>(
        existingInProjectRes.rows
          .map((r: { pmid: string | null }) => r.pmid)
          .filter(Boolean) as string[],
      );
      const existingDoisInProject = new Set<string>(
        existingInProjectRes.rows
          .map((r: { doi: string | null }) => r.doi?.toLowerCase())
          .filter(Boolean) as string[],
      );
      ...
      const searchMultiplier = Math.min(
        5,
        Math.max(2, Math.ceil(existingCount / bodyP.data.maxResults) + 1),
      );
      const maxPerSource =
      ...
      if (openrouterKey && dedupedArticles.length > 0) {
        ...
        try {
          if (featureFlags.aiAsSignal) {
            ...
            // scoreResult.failed tracked via fail-safe logic
            ...
            // filterResult.failed tracked via fail-safe logic
          ...
          // AI scoring failed — fail-safe keeps all articles
      ...
      for (const [source, result] of Object.entries(sourceResults)) {
        const sourceName =
          source === "pubmed" ? "PubMed" : source === "doaj" ? "DOAJ" : "Wiley";
        sourceBreakdown.push(
          `${sourceName}: ${result.added} из ${result.count}`,
        );
      }
    ...
    async (request, reply) => {
      ...
      try {
        if (bodyP.data.useParallel) {
          ...
          const result = await translateArticlesParallel(
            openrouterKey,
            toTranslate.rows,
            {
              parallelCount: bodyP.data.parallelCount,
          ...
          for (let i = 0; i < toTranslate.rows.length; i += BATCH_SIZE) {
            ...
            sendEvent("progress", {
              ...
              total: toTranslate.rows.length,
              percent: Math.round(
                ((i + batch.length) / toTranslate.rows.length) * 100,
              ),
              speed: Math.round(speed * 100) / 100,
              eta: Math.round(
                (toTranslate.rows.length - i - batch.length) /
                  Math.max(speed, 0.1),
              ),
            });
          }
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        fastify.log.error(
          { err },
          "[fetch-references] Failed to start pg-boss",
        );
        return reply
          .code(500)
          .send({ error: "Failed to start background job queue" });
      }

      try {
        fastify.log.info(
          { jobId, projectId: paramsP.data.id },
          "[fetch-references] Attempting to send job to pg-boss",
        );
        const jobData = {
          projectId: paramsP.data.id,
          jobId,
        ...
        return reply
          .code(500)
          .send({ error: "Failed to enqueue background job" });
      }
  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      if (job.status === "running" && job.last_progress_at) {
        stalledSeconds = Math.round(
          (Date.now() - new Date(job.last_progress_at).getTime()) / 1000,
        );
        ...
        if (isStalled) {
          ...
          return {
            ...
            errorMessage:
              "Загрузка автоматически отменена: нет прогресса более 60 секунд. Попробуйте снова позже.",
            isStalled: true,
            stalledSeconds,
      ...
      // Вычисляем прогресс
      const progress =
        job.total_articles > 0
          ? Math.round(
              (job.processed_articles / job.total_articles) * 50 +
                (job.fetched_pmids / Math.max(job.total_pmids_to_fetch, 1)) *
                  50,
            )
          : 0;
      ...
      if (
        job.status === "running" &&
        job.error_message?.startsWith("[Phase:")
      ) {
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      if (result.rowCount === 0) {
        return reply
          .code(404)
          .send({ error: "Нет активной загрузки для отмены" });
      }
  ...
  fastify.post(
    ...
    async (request, reply) => {
      ...
      const pmids = Array.from(
        new Set((bodyP.data.pmids || []).map((p) => p.trim()).filter(Boolean)),
      ).slice(0, 100);
    ...
    async (request, reply) => {
      ...
      if (!source) {
        return reply.code(404).send({
          error: "PDF not found",
          message:
            "Не удалось найти PDF. Попробуйте поискать через Google Scholar или на сайте журнала.",
        });
      }
  ...
  fastify.post<{
    ...
    async (request, reply) => {
      ...
      if (!title && !abstract) {
        return reply
          .code(400)
          .send({ ok: false, error: "Nothing to translate" });
      }
      ...
      if (!apiKey) {
        return reply.code(400).send({
          ok: false,
          error:
            "Для перевода нужен API ключ OpenRouter. Добавьте его в Настройках.",
        });
      }
  ...
  // Схема для gaps
  const GapInfoSchema = z.object({
    type: z.string(), // temporal, population, methodological, geographic, etc.
  ...
  const GraphAIAssistantSchema = z.object({
    ...
    clusters: z.array(ClusterInfoSchema).optional(), // Семантические кластеры
    gaps: z.array(GapInfoSchema).optional(), // Выявленные пробелы
  });
    ...
    async (request, reply) => {
      ...
      try {
        ...
        const articlesWithData = externalArticles.filter((a) => {
          ...
          const hasRealTitle =
            a.title &&
            !a.title.startsWith("PMID:") &&
            !a.title.startsWith("DOI:");
        ...
        const graphLevelToType = (level: number | null | undefined): string => {
          switch (level) {
            case 0:
              return "citing"; // Цитирует статью из проекта
            case 1:
              return "project"; // В проекте
            case 2:
              return "reference"; // Ссылка
            case 3:
              return "related"; // Связанная
            default:
              return "unknown";
          }
        };
        ...
        if (filters?.statsQuality && filters.statsQuality > 0) {
          filterDescriptions.push(
            `Мин. качество p-value: ${filters.statsQuality}`,
          );
        }
        ...
        const sourceStats = {
          pubmed: articlesWithData.filter(
            (a) => a.source === "pubmed" || !a.source,
          ).length,
          doaj: articlesWithData.filter((a) => a.source === "doaj").length,
          wiley: articlesWithData.filter((a) => a.source === "wiley").length,
        };

        // Формируем информацию о кластерах
        const clustersInfo =
          clusters && clusters.length > 0
            ? `\nСЕМАНТИЧЕСКИЕ КЛАСТЕРЫ (${clusters.length} шт.):\n` +
              clusters
                .map((c, i) => {
                  const articlesList = c.articleIds.slice(0, 5).join(", ");
                  return `${i + 1}. "${c.name}" - ${c.articleCount} статей${c.centralArticleTitle ? ` (центральная: "${c.centralArticleTitle.substring(0, 50)}...")` : ""}\n   ID статей (первые 5): ${articlesList}${c.articleIds.length > 5 ? "..." : ""}`;
                })
                .join("\n")
            : "";

        // Формируем информацию о gaps
        const gapsInfo =
          gaps && gaps.length > 0
            ? `\nВЫЯВЛЕННЫЕ ПРОБЕЛЫ (GAPS) В ИССЛЕДОВАНИЯХ (${gaps.length} шт.):\n` +
              gaps
                .map((g, i) => {
                  let desc = `${i + 1}. [${g.type.toUpperCase()}] ${g.description}`;
                  if (g.severity) desc += ` (важность: ${g.severity})`;
                  if (g.period) desc += ` - период: ${g.period}`;
                  if (g.articleCount !== undefined)
                    desc += ` - ${g.articleCount} статей`;
                  return desc;
                })
                .join("\n")
            : "";
...
3. Если запрос связан с кластерами - использовать информацию о кластерах для поиска
4. Если запрос связан с gaps - помочь найти статьи для закрытия пробелов
...
- При запросах про кластеры - возвращай статьи из указанного кластера
- При запросах про gaps - ищи статьи, которые могут закрыть пробел
    ...
    async (request, reply) => {
      ...
      const docRes = await pool.query(
        `INSERT INTO documents (project_id, title, content, order_index, created_by)
         VALUES ($1, $2, $3, $4, $5)
         RETURNING id, title`,
        [projectId, title, content, maxOrder.rows[0].next_order, userId],
      );
      ...
      return {
        ...
        bibliographyAdded,
        message:
          `Документ "${title}" создан` +
          (bibliographyAdded > 0
            ? `. Добавлено ${bibliographyAdded} ссылок из библиографии`
            : ""),
      };
    },
  );
