...
const plugin: FastifyPluginAsync = async (fastify) => {
  const hasColumn = async (
    ...
    try {
      ...
      logError(fastify.log, "documents.graph.hasColumn", error, {
        tableName,
        columnName,
      });
      return false;
  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      const hasSubNumber = await hasColumn("citations", "sub_number");
  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      const hasVolumeColumns = await hasColumn("articles", "volume");
    ...
    async (request, reply) => {
      ...
      if (query.sourceQueries) {
        try {
          ...
          logError(fastify.log, "documents.graph.parseSourceQueries", error, {
            sourceQueries: query.sourceQueries,
          });
          sourceQueries = [];
        }
      ...
      if (query.sources) {
        try {
          ...
          logError(fastify.log, "documents.graph.parseSourcesFilter", error, {
            sources: query.sources,
          });
          sourcesFilter = [];
        }
      ...
      const [hasRefColumns, hasSourceQueryCol, hasStatsQuality] =
        await Promise.all([
          hasColumn("articles", "reference_pmids"),
          hasColumn("project_articles", "source_query"),
          hasColumn("articles", "stats_quality"),
        ]);
      ...
      const hasRefDoisCol = await hasColumn("articles", "reference_dois");
      ...
      for (const article of allArticles) {
        ...
        if (Array.isArray(article.reference_pmids)) {
          ...
          try {
            ...
            logError(
              fastify.log,
              "documents.graph.parseReferencePmids",
              error,
              {
        ...
        if (Array.isArray(article.cited_by_pmids)) {
          ...
          try {
            ...
            logError(fastify.log, "documents.graph.parseCitedByPmids", error, {
              articleId: article.id,
            });
            citedByPmids = [];
          }
        ...
        if (Array.isArray(article.reference_dois)) {
          ...
          try {
            ...
            logError(fastify.log, "documents.graph.parseReferenceDois", error, {
              articleId: article.id,
            });
            refDois = [];
          }
  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        const hasRefPmids = await hasColumn("articles", "reference_pmids");
