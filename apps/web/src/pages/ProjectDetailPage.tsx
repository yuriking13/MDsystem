...
export default function ProjectDetailPage() {
    ...
    return (
      <SidebarLayout>
        <div className="flex items-center justify-center py-12">
          ...
          <span className="ml-3 text-gray-500 dark:text-gray-400">Загрузка проекта...</span>
        </div>
      </SidebarLayout>
    );
  }
    ...
    return (
      <SidebarLayout>
        <div className="max-w-screen-xl mx-auto">
          <div className="p-4 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400 mb-4" role="alert">
            {error || "Проект не найден"}
          </div>
          <button
            onClick={() => nav("/projects")}
            type="button"
            className="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-primary-700 rounded-lg hover:bg-primary-800"
          >
            ← К проектам
          </button>
        </div>
  ...
  return (
    <SidebarLayout projectId={id} projectName={project.name}>
      <div className="max-w-screen-xl mx-auto">
        {/* Header */}
        <div className="mb-6">
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            {project.role === 'owner' ? 'Владелец' : project.role} • Обновлён: {new Date(project.updated_at).toLocaleDateString('ru-RU')}
          </p>
        </div>
