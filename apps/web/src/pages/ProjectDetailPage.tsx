...
> = {
  observational_descriptive: {
    ...
    subtypes: [
      {
        value: "case_report",
        name: "Клинический случай (Case Report)",
        description: "Описание отдельного случая",
      },
      {
        value: "case_series",
        name: "Серия случаев",
        description: "Описание нескольких схожих случаев",
      },
    ],
  },
    ...
    subtypes: [
      {
        ...
        name: "Когортное проспективное",
        description: "Наблюдение группы во времени",
      },
      {
        value: "cohort_retrospective",
        name: "Когортное ретроспективное",
        description: "Анализ прошлых данных",
      },
      {
        value: "case_control",
        name: "Случай-контроль",
        description: "Сравнение случаев с контролем",
      },
      {
        value: "cross_sectional",
        name: "Поперечное (одномоментное)",
        description: "Срез в один момент времени",
      },
    ],
  },
  experimental: {
    ...
    subtypes: [
      {
        value: "rct",
        name: "РКИ (рандомизированное контролируемое)",
        description: "Золотой стандарт",
      },
      {
        value: "quasi_experimental",
        name: "Квазиэкспериментальное",
        description: "Без полной рандомизации",
      },
      {
        value: "pre_post",
        name: "Пред- и постэкспериментальное",
        description: "До и после вмешательства",
      },
    ],
  },
  second_order: {
    ...
    subtypes: [
      {
        ...
        description: "Систематический поиск и анализ",
      },
      {
        value: "meta_analysis",
        name: "Метаанализ",
        description: "Статистический синтез результатов",
      },
    ],
  },
  other: {
...
const RESEARCH_PROTOCOLS: Record<
  ResearchProtocol,
  {
    name: string;
    fullName: string;
...
export default function ProjectDetailPage() {
  ...
  const [researchProtocol, setResearchProtocol] = useState<
  ...
  const [statisticsView, setStatisticsView] = useState<"charts" | "tables">(
    "charts",
  );
  const [showCreateStatistic, setShowCreateStatistic] = useState(false);
  const [creatingChartType, setCreatingChartType] = useState<ChartType | null>(
    null,
  );
  ...
  const [filesCategory, setFilesCategory] = useState<FileCategory | "all">(
    "all",
  );
  ...
  const [importStatus, setImportStatus] = useState<"selected" | "candidate">(
    "selected",
  );
  ...
  const [selectedChaptersForExport, setSelectedChaptersForExport] = useState<
    Set<string>
  >(new Set());
  ...
  const handleWSStatisticCreated = useCallback(
  ...
  const handleWSStatisticUpdated = useCallback(
    (statistic: ProjectStatistic) => {
      if (!statistic) return;
      setStatistics((prev) =>
        prev.map((s) => (s.id === statistic.id ? statistic : s)),
      );
  ...
  const [bibliographyLastUpdated, setBibliographyLastUpdated] =
    useState<number>(0);
  ...
  async function load() {
    ...
    try {
      ...
      setAiErrorAnalysisEnabled(
        pRes.project.ai_error_analysis_enabled || false,
      );
  ...
  useEffect(() => {
    if (activeTab !== "statistics" || editingStat || showCreateStatistic)
      return;
  ...
  async function handleDeleteStatistic(statId: string) {
    ...
    try {
      ...
      if (
        getErrorMessage(err)?.includes("используется") ||
  ...
  async function handleUpdateStatistic(
    ...
    try {
      ...
      if (
        getErrorMessage(err)?.includes("VersionConflict") ||
        getErrorMessage(err)?.includes("изменены другим")
      ) {
    ...
    try {
      ...
      const preparedDocuments = await Promise.all(
        res.documents.map(async (d) => ({
          title: d.title,
          content: d.content
            ? await prepareHtmlForExport(
                d.content,
                chartImages,
                includeChartDataTables,
              )
            : null,
        })),
      );
  ...
  async function handleExportSelectedChapters() {
    ...
    try {
      ...
      const selectedDocs = res.documents.filter((d) =>
        selectedChaptersForExport.has(d.id),
      );
      ...
      const selectedBibliography = res.bibliography.filter((bib) => {
        ...
        return selectedDocs.some((d) => {
          ...
          return (
            content.includes(`data-citation-id="${bib.articleId}"`) ||
  ...
  function handleCopyBibliography() {
    const text = bibliography
      .map((item) => `${item.number}. ${item.formatted}`)
      .join("\n");
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      {loadingBib || updatingBibliography
                        ? "Обновление..."
                        : "Обновить"}
                    </button>
                    <button
              ...
              {!storageConfigured && (
                ...
                >
                  ...
                  <p className="muted" style={{ margin: 0 }}>
                    Для загрузки файлов необходимо настроить подключение к
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="file-info">
                              ...
                              {file.usedInDocuments &&
                                file.usedInDocuments.length > 0 && (
                                  <div className="file-usage-tags">
                                    {file.usedInDocuments.map((docId) => {
                                      ...
                                      return doc ? (
                                        <span
                                          key={docId}
                                          className="file-usage-tag"
                                          title={`Используется в: ${doc.title}`}
                                        >
                                          📄{" "}
                                          {doc.title.length > 15
                    ...
                    <div className="modal-body" style={{ padding: 20 }}>
                      ...
                      <label className="stack" style={{ marginBottom: 12 }}>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateImportMetadata("title", e.target.value)
                          }
                      ...
                      <label className="stack" style={{ marginBottom: 12 }}>
                        ...
                        <input
                          type="text"
                          value={
                            fileImportModal.metadata.authors?.join(", ") || ""
                          }
                          onChange={(e) =>
                          ...
                          <div style={{ marginBottom: 16 }}>
                            ...
                            >
                              <span style={{ fontWeight: 500 }}>
                                ...
                                Библиография (
                                {fileImportModal.metadata.bibliography.length}{" "}
                                ссылок)
                              </span>
                            </div>
                            <div
                              ...
                              {fileImportModal.metadata.bibliography
                                ...
                                .map((ref, idx) => (
                                  ...
                                  >
                                    ...
                                    {ref.authors && (
                                      <div className="muted">{ref.authors}</div>
                                    )}
                                    {ref.year && (
                                      <div className="muted">
                                        Год: {ref.year}
            ...
            <div className="project-section-body statistics-page">
              <div className="statistics-controls" style={{ marginBottom: 16 }}>
                ...
                <button
                  ...
                  onClick={async () => {
                    if (!id) return;
                    if (
                      !confirm("Удалить статистики без данных (битые записи)?")
                    )
                      return;
                    try {
              ...
              {loadingStats ? (
                ...
                <div className="statistics-empty">
                  ...
                  <p className="muted">
                    Создайте графики из таблиц в документах проекта.
                    <br />
                    ...
                    return (
                      ...
                      >
                        {!isValidId && (
                          ...
                          >
                            ⚠️ Поврежденная запись: невалидный ID.
                            Редактирование недоступно, но можно удалить.
                          </div>
                        )}
                          ...
                          {!showAsTable &&
                            ...
                            (!stat.config || !stat.config.type) && (
                              <div
                                ...
                                style={{ color: "#ff6b6b" }}
                              >
                                ⚠️ Конфигурация графика отсутствует или
                                повреждена
                              </div>
                            )}
                        ...
                        <div className="stat-card-actions">
                          <button
                            ...
                            title={
                              !isValidId
                                ? "Редактирование недоступно для поврежденных записей"
                                : "Редактировать"
                            }
                            disabled={!isValidId}
                          >
                                    ...
                                    canvas.toBlob(async (blob) => {
                                      if (blob) {
                                        try {
                                          ...
                                          setOk(
                                            "График скопирован как изображение!",
                                          );
                                        } catch {
