...
export function useProjectWebSocket({
  ...
  }, [onStatisticCreated, onStatisticUpdated, onStatisticDeleted, onDocumentUpdated, onDocumentDeleted, onEvent]);
  ...
  const connect = useCallback(() => {
      ...
      try {
        ...
        switch (data.type) {
          case "statistic:created":
            callbacksRef.current.onStatisticCreated?.(data.payload?.statistic);
            break;
          case "statistic:updated":
            callbacksRef.current.onStatisticUpdated?.(data.payload?.statistic);
            break;
          case "statistic:deleted":
            callbacksRef.current.onStatisticDeleted?.(data.payload?.statisticId);
            break;
          case "document:updated":
            callbacksRef.current.onDocumentUpdated?.(data.payload?.document);
            break;
          case "document:deleted":
            callbacksRef.current.onDocumentDeleted?.(data.payload?.documentId);
            break;
          case "connected":
    ...
    ws.onclose = (event) => {
      ...
      if (enabled && event.code !== 4001) { // 4001 = invalid token, не переподключаемся
