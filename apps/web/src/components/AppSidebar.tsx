...
export default function AppSidebar({
  ...
  const isDocumentEditorRoute = /^\/projects\/[^/]+\/documents\/[^/]+$/.test(
    location.pathname,
  );
  const [collapsed, setCollapsed] = useState(() => isDocumentEditorRoute);
  ...
  // In document editor route we start from collapsed navigation on desktop.
  useEffect(() => {
    if (mobileViewport) return;
    if (isDocumentEditorRoute) {
      setCollapsed(true);
    }
  }, [isDocumentEditorRoute, mobileViewport]);
      ...
      <div className="sidebar-footer">
        ...
        >
          {isCollapsedView ? (
            <button
              type="button"
              className="sidebar-theme-toggle-collapsed"
              onClick={toggleTheme}
              aria-label={
                isDark
              ...
              data-testid="sidebar-theme-toggle-collapsed"
            >
              {isDark ? (
                <svg
                  className="sidebar-theme-toggle-collapsed__icon"
