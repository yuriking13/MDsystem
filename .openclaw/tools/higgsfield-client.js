...
class HiggsFieldDevTool {
  ...
  async generateForDevelopment(prompt, options = {}) {
    ...
    console.log(`📐 Format: ${requestData.aspect_ratio} @ ${requestData.resolution}`);
  ...
  async generateLandingContent() {
    const prompts = [
      {
        ...
        prompt: 'Abstract scientific background with molecular structures, DNA helix, neural networks, professional medical blue theme',
        aspectRatio: '16:9'
      },
      {
        name: 'ai-brain-visualization',
        prompt: 'Artistic visualization of AI brain, neural network connections, synapses, futuristic medical technology',
        aspectRatio: '1:1'
      },
      {
        name: 'research-workflow',
        prompt: 'Scientific research workflow diagram, laboratory equipment, data analysis, research process illustration',
        aspectRatio: '16:9'
      },
      {
        name: 'medical-innovation',
        prompt: 'Modern medical research, combining traditional medicine with AI technology, innovation in healthcare',
        aspectRatio: '4:3'
      }
    ];
    ...
    for (const item of prompts) {
      try {
        ...
        const downloads = await this.downloadImages(images, `./generated-content/landing/${item.name}`);
  ...
  async generateUIElements() {
    const elements = [
      {
        name: 'feature-icons',
        prompt: 'Set of scientific feature icons: microscope, DNA, brain, graph, molecule, lab equipment, minimalist style',
        aspectRatio: '1:1'
      },
      {
        name: 'service-illustrations',
        prompt: 'Medical service illustrations: patient care, research analysis, data processing, telemedicine',
        aspectRatio: '4:3'
      }
    ];
    ...
    for (const element of elements) {
      try {
        ...
        const downloads = await this.downloadImages(images, `./generated-content/ui/${element.name}`);
  ...
  switch (command) {
    ...
    case 'custom':
      ...
      if (!prompt) {
        ...
        console.log('Usage: node higgsfield-client.js custom "your prompt here"');
        process.exit(1);
      }
    ...
    default:
      ...
      console.log('');
      console.log('Usage:');
      console.log('  node higgsfield-client.js landing    - Generate landing page content');
      console.log('  node higgsfield-client.js ui         - Generate UI elements');
      console.log('  node higgsfield-client.js custom "prompt" - Custom generation');
      console.log('');
