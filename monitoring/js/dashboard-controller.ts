...
class DashboardController {
  ...
  restartAllAgents() {
    if (
      confirm(
        "Are you sure you want to restart all agents? This will interrupt any running tasks.",
      )
    ) {
      if (window.wsClient) {
  ...
  performExport() {
    ...
    if (
      document.getElementById("export-tasks").checked &&
      window.tableManager.taskTable
    ) {
      exportData.tasks = window.tableManager.taskTable.getData();
    }

    if (
      document.getElementById("export-errors").checked &&
      window.tableManager.errorTable
    ) {
      exportData.errors = window.tableManager.errorTable.getData();
    }

    if (
      document.getElementById("export-agents").checked &&
      window.tableManager.agentTable
    ) {
      exportData.agents = window.tableManager.agentTable.getData();
    }

    if (
      document.getElementById("export-metrics").checked &&
      window.chartManager
    ) {
      exportData.metrics = {
  ...
  exportAsExcel(data, filename) {
    ...
    this.showNotification(
      "Excel export not fully implemented. CSV exported instead.",
      "warning",
    );
  }
  ...
  healthCheck() {
    const health = {
      websocket: window.wsClient
        ? window.wsClient.connectionState
        : "not_initialized",
