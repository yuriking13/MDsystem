...
class DataSimulator {
  ...
  startSimulation() {
    ...
    console.log(
      "Simulation started with intervals:",
    ...
    setTimeout(
      () => {
        if (Math.random() > 0.3) {
          // 70% chance to start
          const agent =
            this.agentNames[Math.floor(Math.random() * this.agentNames.length)];
  ...
  simulateError() {
    ...
    const source =
      this.errorSources[Math.floor(Math.random() * this.errorSources.length)];
  ...
  getRandomCapabilities() {
    ...
    for (let i = 0; i < count; i++) {
      const capability =
        allCapabilities[Math.floor(Math.random() * allCapabilities.length)];
...
if (require.main === module) {
  ...
  const speed =
    args.find((arg) => arg.startsWith("--speed="))?.split("=")[1] || "1";
  const duration =
  ...
  process.stdin.on("readable", () => {
    ...
    if (chunk !== null) {
      ...
      switch (command) {
        ...
        case "help":
          console.log(
            "Available commands: status, stop, start, restart, quit/exit, help",
          );
          break;
        default:
