...
class MonitoringServer {
  ...
  async collectSystemMetrics() {
    try {
      ...
      const recentTasks = Array.from(this.taskQueue.values()).filter(
        (task) => task.updated > oneMinuteAgo,
      );
      ...
      const errorRate =
        totalRecentEvents > 0
          ? (recentErrors.length / totalRecentEvents) * 100
          : 0;
      ...
      this.systemMetrics = {
        ...
        agents: {
          ...
          online: Array.from(this.agents.values()).filter(
            (a) => a.status === "online",
          ).length,
        },
        tasks: {
          total: this.taskQueue.size,
          pending: Array.from(this.taskQueue.values()).filter(
            (t) => t.status === "pending",
          ).length,
          processing: Array.from(this.taskQueue.values()).filter(
            (t) => t.status === "processing",
          ).length,
          completed: Array.from(this.taskQueue.values()).filter(
            (t) => t.status === "completed",
          ).length,
          failed: Array.from(this.taskQueue.values()).filter(
            (t) => t.status === "failed",
          ).length,
        },
      };
  ...
  checkOfflineAgents() {
    ...
    this.agents.forEach((agent, id) => {
      if (
        now - agent.lastSeen > offlineThreshold &&
  ...
  start() {
    this.server.listen(this.port, () => {
      ...
      console.log(
        `WebSocket endpoint: ws://localhost:${this.port}/ws/monitoring`,
      );
    });
  }
